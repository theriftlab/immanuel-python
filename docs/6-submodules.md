...
| date | Timezone management based on geographical coordinates, and easy conversion between Gregorian and Julian dates across timezones. Also provides supporting function including Delta-T and sidereal time. |
...
| sweph | The direct interface with the `swisseph` module, standardizing its house, angle, fixed star and other chart object data for the other `tools` modules to build on. Most of its functions can work with either a Julian date or an ARMC. This is the main ephemeris engine at the core of Immanuel. |
