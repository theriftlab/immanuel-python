...
def all(items1: dict, items2: dict, pars_fortuna: int = calc.COMPOSITE, pars_fortuna_formula: int = None) -> dict:
    ...
    if pars_fortuna == calc.COMPOSITE and chart.PARS_FORTUNA in items1:
        items[chart.PARS_FORTUNA] = {
            'index': chart.PARS_FORTUNA,
            'type': chart.POINT,
            'name': names.POINTS[chart.PARS_FORTUNA],
            'lon': calculate.pars_fortuna(items[chart.SUN]['lon'], items[chart.MOON]['lon'], items[chart.ASC]['lon'], pars_fortuna_formula),
            'speed': 0.0,
        }
