...
def chart_shape(objects: dict, chart_shape_orb: float) -> int:
    ...
    if max_diff >= 240-chart_shape_orb:
        return calc.BUNDLE
    ...
    for k, v in enumerate(diffs):
        ...
        if v >= 90-chart_shape_orb and (next >= 90-chart_shape_orb or (next <= chart_shape_orb and second_next >= 90-chart_shape_orb)):
            return calc.BUCKET

    # All planets being within 180º with no bucket handle means a bowl
    if max_diff >= 180-chart_shape_orb:
        return calc.BOWL

    # All planets being within 240º with no bucket handle means a locomotive
    if max_diff >= 120-chart_shape_orb:
        return calc.LOCOMOTIVE

    diffs.sort()

    # Only two gaps of at least 60º mean a seesaw
    if len([v for v in diffs if v >= 60-chart_shape_orb]) == 2:
        return calc.SEESAW

    # Three gaps of at least 30º mean a splay
    if len([v for v in diffs if v >= 30-chart_shape_orb]) == 3:
        return calc.SPLAY
